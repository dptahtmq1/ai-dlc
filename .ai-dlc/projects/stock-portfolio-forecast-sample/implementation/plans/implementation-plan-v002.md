...
- Python 패키지와 테스트는 `uv` 잠금 파일로 재현하고, Node 패키지는 프론트엔드 잠금 파일로 고정한다. 잠금 파일 종류는 최초 패킷에서 생성되는 Vite 프로젝트의 패키지 관리자에 맞추되 새 선택을 추가로 발명하지 않는다.
...
- 테스트 및 수용 증거: `uv run pytest backend/tests/contracts`와 프론트엔드 Vitest 명령으로 계약 직렬화 왕복, 미지원 버전·필드 거절, 상태 전이와 API 표현 모델 fixture를 자동화한다. 두 잠금 파일의 재설치 결과도 남긴다.
...
- 허용 경로/컴포넌트: `backend/src/stock_analysis/input/`, `data/`, `portfolio/current/`, `backend/tests/`, `fixtures/`의 합성 어댑터 자료.
...
- 테스트 및 수용 증거: `uv run pytest backend/tests -k "input or current or data"`로 정상·빈 값·중복·잘못된 수량·한도 초과, 독립 기준 계산, 미래 데이터 거절과 네 가지 데이터 상태를 검증한다.
- 마이그레이션/호환성: 공급자 원본 형식은 어댑터 밖으로 노출하지 않고 내부 계약 변경은 `WP-001`의 버전 규칙을 따른다.
- 롤백 경계: `input/`, `data/`, `portfolio/current/`와 대응 시험만 되돌리며 계약 골격은 유지한다.
...
- 허용 경로/컴포넌트: `backend/src/stock_analysis/analysis/eligibility/`, `forecast/`, `backtest/`, 모델·정책 구성, `backend/tests/`, `fixtures/`.
...
- 테스트 및 수용 증거: `uv run pytest backend/tests -k "eligibility or forecast or backtest or leakage"`로 적격·부적격·누락 표본, 결정적 예측, 입력 부족 실패, 시간 경계 분할과 미래 데이터 누수 0건을 증빙한다.
- 마이그레이션/호환성: 모델·정책·백테스트 버전은 불변 참조로 기록하며 교체 시 새 버전과 회귀 fixture를 요구한다.
- 롤백 경계: `analysis/`와 해당 버전 구성을 함께 제거하고 `WP-002` 기능은 유지한다.
- 상태: `PROPOSED_BLOCKED_BY_OPEN_DECISION`
...
- 허용 경로/컴포넌트: `backend/src/stock_analysis/portfolio/target/`, `policy/`, `risk/`, 대응 `backend/tests/`와 `fixtures/`.
...
- 테스트 및 수용 증거: `uv run pytest backend/tests -k "target or policy or risk or rebalance"`로 합계 100%, 적격 후보 한정, 반올림 경계, 제약 충족·충돌·해 없음과 후보 포함 정책 양쪽 사례를 검증한다.
- 마이그레이션/호환성: 정책 변경은 새 버전으로 추가하고 기존 실행 참조를 덮어쓰지 않는다.
- 롤백 경계: `portfolio/target/`, `policy/`, `risk/`와 정책 버전만 되돌리며 앞선 기능은 유지한다.
- 상태: `PROPOSED_BLOCKED_BY_OPEN_DECISION`
...
- 허용 경로/컴포넌트: `backend/src/stock_analysis/orchestration/`, `evidence/`, 개발 저장 어댑터, `backend/tests/`와 `fixtures/`.
...
- 테스트 및 수용 증거: `uv run pytest backend/tests -k "orchestration or evidence or idempotent or recovery"`로 정상 전이, 중단·저장 실패·필드 누락·부분 기록, 멱등 반복, 충돌 쓰기와 재현 수치 비교를 검증한다.
- 마이그레이션/호환성: 저장 형식 버전, 최소 보존 범위와 정리 절차는 `OPEN-IMPL-011`에서 확정한다.
- 롤백 경계: `orchestration/`, `evidence/`와 합성 개발 기록만 정리하며 기준선과 fixture 원본은 보존한다.
- 상태: `PROPOSED_BLOCKED_BY_OPEN_DECISION`
...
- 테스트 및 수용 증거: Vitest/RTL 명령으로 컴포넌트 상태와 키보드 흐름을 검증하고, `npx playwright test`로 종단 간 흐름을 검증한다. `@axe-core/playwright` 위반 0건, 결정된 화면 크기에서 겹침·잘림·색상 단독 표현 0건의 증거를 남긴다.
...
|---|---|---|---|---|
| `TEST-001` | `WP-001` | `DEC-001`~`DEC-004`, `INT-001`~`INT-006` | `uv run pytest backend/tests/contracts`; 프론트엔드 Vitest | 계약 왕복, 미지원 버전/전이 거절, 잠금 재현 |
| `TEST-002` | `WP-002` | `FR-022`, `AC-029` | `uv run pytest backend/tests -k "input or current"` | 가치·비중·총가치 기준값과 필드별 오류 |
| `TEST-003` | `WP-002`, `WP-004` | `NFR-012`, `QA-012`, `AC-034` | pytest 독립 기준 계산 대조 | 결정된 허용오차 내 일치와 합계 100% |
| `TEST-004` | `WP-002`, `WP-003` | `NFR-013`, `QA-013`, `AC-035` | `uv run pytest backend/tests -k leakage` | 기준 시점 이후 데이터 사용 위반 0건 |
| `TEST-005` | `WP-003` | `FR-023`, `AC-030` | pytest 적격·부적격·누락 fixture | 판정, 근거, 정책 버전과 제외 결과 |
| `TEST-006` | `WP-003` | `FR-024`, `AC-031` | pytest 고정 scikit-learn 예측·백테스트 | 정상 산출 필드와 입력 부족 실패 원인 |
| `TEST-007` | `WP-004` | `FR-025`, `AC-032` | pytest 정책 충족·충돌·해 없음 시험 | 적격 후보 한정 목표안 또는 충돌 식별 |
| `TEST-008` | `WP-005` | `NFR-014`, `QA-014`, `AC-036`, `ADR-003` | pytest 중단·부분·멱등·충돌 주입 | 메타데이터 누락 0건, 완료 오인 0건 |
| `TEST-009` | `WP-002`, `WP-005`, `WP-006` | `NFR-015`, `QA-015`, `AC-037` | pytest와 Vitest 상태 fixture 공유 | 상태·기준 시각·원인·영향 영역 일치 |
| `TEST-010` | `WP-006` | `FR-026`, `AC-033` | Vitest/RTL 컴포넌트·사용자 흐름 | 현재·목표·분석 근거와 비정상 상태 구분 |
| `TEST-011` | `WP-006` | `NFR-016`, `QA-016`, `AC-038` | `npx playwright test`, `@axe-core/playwright` | 접근성 위반, 겹침, 잘림, 색상 단독 표현 0건 |
| `TEST-012` | `WP-002`~`WP-006` | `FR-022`~`FR-026` | FastAPI+Vite 로컬 실행 후 Playwright | 실행 ID에 요청부터 표현 모델까지 연결된 재현 증거 |

전체 백엔드 회귀는 `uv run pytest backend/tests`, 프론트엔드 단위 회귀는 프론트엔드 매니페스트에 고정된 Vitest 스크립트, 브라우저 회귀는 `npx playwright test`로 실행한다. `OPEN-IMPL-001`~`009`, `011`이 필요한 시험은 임의 기본값으로 합격 처리하지 않는다.
