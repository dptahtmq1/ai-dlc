...
AI-DLC는 Codex Plugin 기반의 멀티 에이전트 워크플로입니다. 사용자의 아이디어를 간결한 요구사항, 아키텍처, 구현 계획, 코드와 검증 증거로 단계적으로 발전시키면서 중요한 결정과 산출물 이력을 유지합니다.
...
각 구현 작업은 승인된 요구사항, 아키텍처와 구현 계획을 입력으로 사용합니다. 변경 범위와 허용 경로, 완료 조건, 검증 명령, 롤백 경계가 명시된 작은 작업 패킷 단위로 수행합니다.
...
- **Local references over matrices:** 필요한 결정과 작업 패킷에서 관련 요구사항을 직접 참조하되, 중복 추적표는 만들지 않습니다.
- **Concise by default:** 필수 정보만 작성하며 빈 섹션, 중복 표와 형식 채우기용 항목을 만들지 않습니다.
...
| `draft-requirements` | FR, 측정 가능한 NFR과 제약사항을 간결하게 작성 |
| `review-requirements` | 요구사항의 완전성, 명확성, 테스트 가능성, 항목 수와 중복 독립 검토 |
| `design-architecture` | 승인된 요구사항을 기반으로 핵심 결정, 구성요소와 인터페이스 작성 |
| `review-architecture` | 품질 속성, 경계, 보안, 운영성과 기술 선택 독립 검토 |
| `plan-implementation` | 작업 패킷, 의존성, 검증, 배포와 롤백 계획 작성 |
| `review-implementation-plan` | 구현 계획의 baseline coverage, 실행 가능성, 범위와 검증 가능성 독립 검토 |
...
- 어떤 요구사항이 관련 설계 결정과 작업 패킷에서 참조됐는지
...
- 관리 항목은 기능 요구사항 `FR-*`, 품질 측정값을 포함한 비기능 요구사항 `NFR-*`, 제약사항 `CON-*` 세 종류입니다.
- FR, NFR과 제약사항은 각각 5~7개를 기본 범위로 합니다.
- 7개를 초과하면 통합할 수 없는 구체적인 근거를 명시합니다. 실제 항목이 5개보다 적으면 형식 채우기용 항목을 만들지 않고 그 이유를 기록합니다.
- 별도 `QA-*`, `AC-*` 항목과 요구사항 추적표는 만들지 않습니다. 검증 조건은 FR에, 운영 조건과 측정 목표는 NFR에 직접 작성합니다.
- 아키텍처와 구현 계획도 중복 매트릭스보다 해당 결정·작업 패킷의 직접 참조를 사용합니다.
...
자세한 상태 머신, 승인 문구, 완료 조건, 리스크 정책과 아키텍처 결정은 [시스템 명세](docs/system-specification.md)를 참고하세요.
