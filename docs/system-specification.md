...
Implementation Plan loop는 사람 승인된 requirements baseline과 architecture baseline을 모두 입력으로 사용해야 한다. 계획 작성자는 work packet, dependency order, 허용 및 금지 범위, acceptance evidence, test, migration, rollout과 rollback을 제안한다. 독립 계획 리뷰어는 다음 항목을 모두 확인한다.

- 모든 필수 baseline 항목이 하나 이상의 work packet과 verification evidence로 추적되는지
...
- Environment: `AWAITING_REQUIREMENTS_LOOP_AUTHORIZATION`
...
- Environment: 승인된 AI loop의 author artifact 생성 이후
...
새 프로젝트에서 AI-DLC를 시작하면 시스템은 프로젝트 ID와 대상 루트를 포함한 initialization proposal을 표시하고 `APPROVE INTAKE PROJECT <project-id>` 전에는 레지스트리나 프로젝트 루트를 생성하지 않아야 한다.

### AC-SYS-002: AI Loop Authorization

승인된 brief가 있어도 `APPROVE REQUIREMENTS AI LOOP vNNN`이 없으면 author agent가 draft를 생성하지 않아야 한다.

### AC-SYS-003: AI Review Before Human Approval

draft 생성 후 시스템은 독립 AI review를 자동 실행해야 하며 `PASS_FOR_HUMAN_APPROVAL` 전에는 사람에게 baseline 승인을 요청하지 않아야 한다.
...
### AC-SYS-006: Bounded Automatic Rework

reviewer가 correctable `REWORK_REQUIRED`를 반환하고 승인된 loop capacity가 남아 있으면 별도 사람 승인 없이 새 immutable revision을 만들 수 있어야 한다. scope expansion, escalation 또는 한도 도달 시에는 새 revision을 만들지 않아야 한다.
...
### AC-SYS-014: Implementation Plan AI Review Loop

승인된 requirements와 architecture baseline이 존재해도 `APPROVE IMPLEMENTATION PLAN AI LOOP vNNN`이 없으면 구현 계획 작성자를 호출하지 않아야 한다. Loop 승인 후에는 `$plan-implementation`과 별도 invocation의 `$review-implementation-plan`을 순서대로 실행하고, correctable `REWORK_REQUIRED`는 최대 iteration 한도 안에서 자동 재작성으로 routing해야 한다. `PASS_FOR_HUMAN_APPROVAL` 전에는 사람에게 계획 baseline 승인을 요청하지 않아야 하며, `APPROVE IMPLEMENTATION PLAN BASELINE vNNN` 전에는 work-packet 구현 단계로 전환하지 않아야 한다.
