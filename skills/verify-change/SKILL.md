...
Require the orchestrator-supplied `project_id` and `project_root` and reject cross-project baselines or evidence. Run only authorized non-destructive checks. Record environment, command, exit status, relevant output, artifact path, and baseline traces. Distinguish product failure, test defect, environment failure, and not-run. Never reinterpret a failure as pass or silently skip a required check. Write `<project_root>/implementation/verifications/WP-NNN-verification-vNNN.md`.
